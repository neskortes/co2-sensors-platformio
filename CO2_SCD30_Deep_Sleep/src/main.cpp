...
#include <LittleFS.h>
static const char* LOG_PATH = "/co2_log.csv";
...
bool initLogFS() {
  ...
  if (!LittleFS.exists(LOG_PATH)) {
    ...
    if (!f) {
      Serial.println("[FS] Failed to create log file");
      return false;
    }
    f.println("ms,co2_ppm,temp_c,rh_pct,ts_sent");
    f.close();
    Serial.println("[FS] Created log file with header");
  } else {
    Serial.println("[FS] Log file exists");
  }
  return true;
}

bool appendLogLine(uint32_t ms, float co2ppm, float tempC, float rhPct, bool tsSent) {
  File f = LittleFS.open(LOG_PATH, "a");
  if (!f) {
    Serial.println("[FS] Failed to open log for append");
    return false;
  }
  f.printf("%lu,%.1f,%.2f,%.1f,%d\n",
           (unsigned long)ms, co2ppm, tempC, rhPct, tsSent ? 1 : 0);
  f.close(); // close = flush
  return true;
}

// --- HTTP server (download log) ---
#include <WebServer.h>
WebServer server(80);
...
void setupHttpRoutes() {
  ...
  Serial.println("[HTTP] Endpoints: /  /log  /clear");
  Serial.print("[HTTP] Try: http://");
...
void wifiConnectDebug() {
  ...
  if (finalS == WL_CONNECTED) {
    ...
    // Start HTTP endpoints once WiFi is up
    setupHttpRoutes();
  } else {
...
void setup() {
  ...
  Serial.println("\nBOOT: SCD30 + WiFi + ThingSpeak + LittleFS log + HTTP /log");
  initLogFS();
  Serial.printf("[FS] total=%u bytes, used=%u bytes, free=%u bytes\n",
...
void loop() {
  // Handle HTTP requests (only does anything once server is started)
  if (httpStarted) {
    server.handleClient();
  }
...
if (now - lastTsSend >= TS_INTERVAL_MS) {
  ...
  // Send once per tick (optional)
  tsSent = sendToThingSpeak(co2, t, rh);

  // Log once per tick
  appendLogLine(now, co2, t, rh, tsSent);
}
  }
